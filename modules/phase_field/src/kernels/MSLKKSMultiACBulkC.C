...
registerMooseObject("PhaseFieldApp", MSLKKSMultiACBulkC);
...
{ // sum over phases
  ...
  for (std::size_t i = 0; i < _nh; ++i)
  {
    ...
    sum += (*_prop_dhdni[i])[_qp] / (*_omega[i])[_qp]  * csum;
  }

  return -_mob[_qp] * _prop_dFdc[_qp] * (*_omega[_l_etai])[_qp] / _a_cs[_l_cs] * sum ; //#####check this line for _omega[_l_etai]
}
...
MSLKKSMultiACBulkC::precomputeQpJacobian()
{
  ...
  for (std::size_t i = 0; i < _nh; ++i)
  {
    ...
    sum += (*_prop_d2hdnidn[i][_l_etai])[_qp] / (*_omega[i])[_qp] * csum ;
  }

  return -_mob[_qp] * _phi[_j][_qp] * _prop_dFdc[_qp] * (*_omega[_l_etai])[_qp] / _a_cs[_l_cs] * sum; //same thing with omega[_l_etai]
}
...
MSLKKSMultiACBulkC::computeQpOffDiagJacobian(unsigned int jvar)
{
    ...
    if (_phase[csvar] == _phase[_l_cs])
    {
      ...
      return -_mob[_qp] *
             ((*_prop_d2Fdcdcs[csvar])[_qp] * sum +
              _prop_dFdc[_qp] * (*_prop_dhdni[_phase[csvar]])[_qp] * _a_cs[csvar] / (*_omega[_phase[csvar]])[_qp]) * (*_omega[_l_etai])[_qp] /
             _a_cs[_l_cs] * _phi[_j][_qp] * _test[_i][_qp];
    }

    return -_mob[_qp] * _prop_dFdc[_qp] * (*_omega[_l_etai])[_qp] / _a_cs[_l_cs] * (*_prop_dhdni[_phase[csvar]])[_qp] / (*_omega[_phase[csvar]])[_qp] *
  ...
  if (etavar >= 0)
  {
    ...
    for (std::size_t i = 0; i < _nh; ++i)
    {
      ...
      sum += (*_prop_d2hdnidn[i][etavar])[_qp] / (*_omega[i])[_qp] * csum;
    }

    return -_mob[_qp] * _prop_dFdc[_qp] * (*_omega[_l_etai])[_qp] / _a_cs[_l_cs] * sum * _phi[_j][_qp] * _test[_i][_qp];
  }
